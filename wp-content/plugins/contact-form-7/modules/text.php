...
function wpcf7_text_validation_filter( $result, $tag ) {
	...
	if ( 'text' == $tag->basetype ) {
		if ( $tag->is_required() && '' == $value && $name == 'your-name' ) {
			$result->invalidate( $tag, wpcf7_get_message( 'invalid_required_name' ) );
		}elseif($tag->is_required() && '' == $value && $name == 'your-subject'){
            $result->invalidate( $tag, wpcf7_get_message( 'invalid_required_sub' ) );
		}elseif( $tag->is_required() && '' == $value){
	...
	if ( 'email' == $tag->basetype ) {
		if ( $tag->is_required() && '' == $value ) {
			$result->invalidate( $tag, wpcf7_get_message( 'invalid_required_email' ) );
