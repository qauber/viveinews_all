...
function view_vendor_order_list_for_front_page(){
  ...
  //$vendor_all_record .="['".date('Y-m-d',$next_next_tomorrow)."', 0],";
  //$vendor_all_record .="['".date('Y-m-d',$next_next_next_tomorrow)."', 0],";
  //$vendor_all_record .="['".date('Y-m-d',$next_next_next_next_tomorrow)."', 0],";
	   ...
	   if( $vendor_records ) { 
        ...
        <div class="wrap stat_display">
          <ul id="tab_ul" class="tabs">
            <li class="selected"><a rel="tab_div1" href="#" onClick="javascript:ShowMyDiv(this);"><?php _e("Statistics","woocommerce-vendor-setup"); ?></a></li>
            <?php
            //if (!is_admin()) {
			if(!current_user_can('edit_theme_options')){
			?>
            <li><a rel="tab_div2" href="#" onClick="javascript:ShowMyDiv(this);"><?php _e("Order List","woocommerce-vendor-setup"); ?></a></li>
            <li><a rel="tab_div3" href="#" onClick="javascript:ShowMyDiv(this);"><?php _e("Payment Status","woocommerce-vendor-setup"); ?></a></li>
            <li><a rel="tab_div4" href="#" onClick="javascript:ShowMyDiv(this);">Published Product</a></li>
            <li><a rel="tab_div5" href="#" onClick="javascript:ShowMyDiv(this);">Pending Product</a></li>
            <?php
			}
			?>
          </ul>
        ...
        <div id="liner_divssss" style="width:600px; height: 500px;" ></div>
...
<!--                <td><?php //echo $ven_record->pro_qty;?></td>-->
        ...
        ?>
           ...
           <!-- ********************************** Payment Status ************************************************** -->
            ...
            <?php if( $vendor_records_details ) { ?>
              ...
              <?php } else { ?>
                    <div style="text-align:center; color:#F00; font-weight:bold; width:100%;"><?php _e("You have no order for pending.","woocommerce-vendor-setup"); ?></div>
                  <?php } ?>
              </div>
              <?php
              $total = $wpdb->get_var( "SELECT count(*) FROM {$wpdb_all_prefix}vendor where vendor_vendor_id = ".$user_ID."  ORDER BY vendor_id DESC");
          ...
          <?php
            ...
            <script type="text/javascript">
				jQuery(document).ready(function(){ 
						jQuery("#orderHistory_Details").tablesorter({widthFixed: false, widgets: ['zebra']}); 
						jQuery("#orderHistory_Details").tablesorterPager({container: jQuery("#author_publish_posts")});			
					} 
				);
            ...
            <?php if( $author_publish_posts ) { ?>
              ...
              <?php } else { ?>
                    <div style="text-align:center; color:#F00; font-weight:bold; width:100%;">You have no product for published yet</div>
                  <?php } ?>
              </div>
              <?php
          ...
          <?php
            ...
            <script type="text/javascript">
				jQuery(document).ready(function(){ 
						jQuery("#orderHistory_Details").tablesorter({widthFixed: false, widgets: ['zebra']}); 
						jQuery("#orderHistory_Details").tablesorterPager({container: jQuery("#author_pending_posts")});			
					} 
				);
            ...
            <?php if( $author_publish_posts ) { ?>
              ...
              <?php } else { ?>
                    <div style="text-align:center; color:#F00; font-weight:bold; width:100%;">You have no product for pending yet</div>
                  <?php } ?>
              </div>
              <?php
	...
	//*********************************************************************************
		...
		?>
			<div style="width: 100%;margin-top: 140px;position: absolute; text-align:center; color:#F00; font-size:14px;"><?php _e("Please wait until admin approval","woocommerce-vendor-setup"); ?></div>
		<?php	
	}
		}
			}
