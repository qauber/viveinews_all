...
// SETTINGS
$uppod_settings['uppod.swf']=plugins_url( '/source/uppod.swf', __FILE__ );     // <-- Flash http://uppod.ru/player/download/
$uppod_settings['uppod.js']=plugins_url( '/source/uppod-0.5.26.js', __FILE__ );      // <-- HTML5 http://uppod.ru/help/html5/

$uppod_settings['swfobject.js']='https://ajax.googleapis.com/ajax/libs/swfobject/2.2/swfobject.js';
$uppod_settings['adobe_update']='Requires player to establish <a href="http://get.adobe.com/flashplayer/" target="_blank">Flash Player</a>';
...
function Uppod($atts, $content = null){
    ...
    if(isset($m)){
    	...
    	if(!$flash && !$html5){
    		$o='Errors, settings Uppod plugin does not contain a link to player (<a href="http://uppod.ru/help/q=wordpress">подробнее</a>)';
    	}
    	else{
			$o=($center?'<center>':'').'
