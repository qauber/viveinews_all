...
if( !class_exists( 'YITH_Woo_Featured_Video_Frontend' ) ) {
    ...
    class YITH_Woo_Featured_Video_Frontend {
        ...
        public function set_featured_video( $html ) {
            ...
            $wp_u_id = md5(uniqid(rand(),1));
            unset( $_SESSION['wb_video'][$wp_u_id] );
            $_SESSION['wb_video'][$wp_u_id] = $this->video_url;
...
-->
               ...
               <div class="product-video site">
               <?php 
               # videojs
               //echo do_shortcode('[videojs_video url="'. get_site_url() .'/wp-video.php?src='. $wp_u_id .'" width="863"]');
               //echo do_shortcode('[videojs_video url="'.$this->video_url.'" width="863"]');
               # uppod
               //echo do_shortcode('[uppod video="'. get_site_url() .'/wp-video.php?src='. $wp_u_id .'" width="863"]');
               echo '<div class="player" id="videoplayer"></div>';
               echo '<script type="text/javascript">this.player = new Uppod({m:"video",uid:"videoplayer",file:"'. get_site_url() .'/wp-video.php?src='. $wp_u_id .'"});</script>';
                ?>

                <!---<video width="640" height="360" controls="controls" tabindex="0">
                <source src="<?php echo get_site_url(); ?>/wp-video.php?src=<?php echo $wp_u_id; ?>" type='video/mp4; codecs="avc1.42E01E, mp4a.40.2"' />
                </video> -->
               </div>
            <?php
            }
